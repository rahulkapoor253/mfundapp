...
class Searchbar extends React.Component {
  ...
  handleInputFund = (event) => {
    ...
    if(event.target.value.length >= 3) {
      axios.get(`https://api.mfapi.in/mf/search?q=${event.target.value}`)
      .then(response => {
        this.setState({
  ...
  return (
    <div>
      ...
      />
        <input type="text" onChange={this.handleInputFund} placeholder="Choose a Mutual fund and see how much wealth you can accumulate over a long period of time"></input>
